...
def plot_feature_importance(norm_coeff_df, features, filename):
    ...
    # Plot bars with specific colors for each model
...
def plot_svr_rf_avg(norm_coeff_df, features, filename):
    ...
    plt.figure(figsize=(16, 8))
    # Use turquoise for the highest importance values and gray for others
    colors = ['turquoise' if val > np.percentile(svr_rf_avg_importance, 80) else 'gray' for val in svr_rf_avg_importance]
    
    # Plot bars for SVR & RandomForest average importance
    plt.bar(x, svr_rf_avg_importance, width, color=colors, label='SVR & RandomForest Avg')
    
    # Adding legend for color explanation
    plt.scatter([], [], color='turquoise', label='Top 20% Importance', s=100)
    plt.scatter([], [], color='gray', label='Other Features', s=100)
    
    plt.xlabel('Feature Index', fontsize=12)
    plt.ylabel('Average Importance', fontsize=12)
    plt.title('SVR & RandomForest Average Feature Importance', fontsize=14)
    plt.xticks(x, range(1, len(features) + 1), rotation=90)
    plt.legend(title="Feature Importance", loc='upper right')
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
